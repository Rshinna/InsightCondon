...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
  ...
  protected void doFilterInternal(
    ...
    if (jwtService.tokenValido(token)) {
      ...
      UUID condominioId = jwtService.extrairCondominioId(token);
      String perfil = jwtService.extrairPerfil(token);

      UsuarioAutenticado usuarioAutenticado =
          new UsuarioAutenticado(usuarioId, condominioId, perfil);

      var authentication =
          new UsernamePasswordAuthenticationToken(
              usuarioAutenticado, null, List.of(new SimpleGrantedAuthority("ROLE_" + perfil)));
